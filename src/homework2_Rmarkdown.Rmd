...
```{r}
# These two events are mutually exclusive, as an individual cannot have two blood types at the same time. The chance of that occuring in a single individual is thus zero.
```
...
## Question 2.2
Out of 100,000 white maWeles in the 20 to 25 age group, how many deaths would you expect in the next 5 years from leukemia?
...
#Alternatively, this could be calculated by finding the probability of the value lying between the mean and standard deviation and the mean - standard deviation P(mean - sd <= X <= mean + sd). Calculating this in R producess a probability of 0.683:
...
```{r}
# Similarly, using the 68-95-99 rule, it can be estimated that the probability of a randomly-selected value falling within two standard deviations of the mean would be around 0.95. By using the pnorm functino to calculate P(mean - 2*SD <= X <= mean + 2*SD) confirms this by returning a probability of 0.954:
...
```{r, echo = TRUE}
# The probability P(mean - 3*SD <= X <= mean + 3*SD) would result in a probability of of 0.997:
...
```{r}
# Since randomization has a binary outcome, this can be calculated with the binomial distribution function pbinom to find the probability P(X > 16), resulting in a probablity of 0.00129. This indicates that it is extremely unlikely that the patients were randomized correctly.
...
```{r}
# Given that the probability of obtaining this sample proportion is so low, it is very unlikely that such a sample proportion would be obtained if the city is representativev of the US population.
```
...
```{r}
##he range that we are 95% sure contains the sample proportion is 0.219 - 0.453.
...
qqplot(x = HTnorm, y = HTbinom, main = "Q-Q Plot of Hypertension Data Quantiles", xlab = "Normal Quantiles", ylab = "Hypertension Sampling Quantiles")
...
```{r}
# The two distributions appear to be very similar, as evidenced by the tendency to cluser around the reference line. Since the comparison sample was taken from a normal distribution, the similarlity of the hypertension sampling distribution indicates that the population being sampled is also normally-distributed.
```
...
```{r}
#Normal distribution with same mean and standard deviation as previous binomial distribution for P(21 <= X <= 24):
...
(pnorm(24,  mean = HTnormVals[1], sd = HTnormVals[2]) - pnorm(20, mean = HTnormVals[1], sd = HTnormVals[2])) %>%
  round(4)
